...
class HomeHandler(BaseHandler):
    ...
    def get(self):
        ...
        if self.is_authorized(['users-editor', 'sudo-users']):
            ...
            self.redirect(f"/attendance/u/{me['user']}")


class AttendanceCalculator:

    def __init__(self, database, user, date: datetime):
    ...
    def _calculate_given_month(self, month: datetime):
        ...
        workspans = adb.get_user_workspans(self.database, self.user, month, next_month)

        has_study_certificate = self._month_has_study_certificate(month)
        has_tax_declaration = self._month_has_tax_declaration(month)
        print("has_study_certificate", has_study_certificate)
        print("has_tax_declaration", has_tax_declaration)
        ...
        for workspan in workspans:
            if workspan["contract"]:
                if not contract:
                    contract = udb.get_user_contract_by_id(self.database, self.user, workspan["contract"])
    ...
    def _month_has_study_certificate(self, month):
        return bool(udb.get_user_active_documents(self.database, self.user, "study_certificate",
                                                  month, month + relativedelta(months=1)))

    def _month_has_tax_declaration(self, month):
        return bool(udb.get_user_active_documents(self.database, self.user, "tax_declaration",
                                                  month, month + relativedelta(months=1)))
    ...
    def month_hour_rate(self):
        ...
        month_contracts = udb.get_user_active_contracts(self.database, self.user, self.month,
...
class ApiAdminMonthTableHandler(BaseHandler):
    ...
    def get(self, date):
        ...
        for user in users:
            calculator = AttendanceCalculator(self.mdb, user["user"], date)
...
class UserAttendanceHandler(BaseHandler):
    ...
    def get(self, user, date_str=None):
        ...
        day_workspans = adb.get_user_workspans(self.mdb, user, date, date + timedelta(days=1))
        ...
        current_and_future_vacations = adb.get_user_vacations(self.mdb, user, date)
...
class ApiMonthInfoHandler(BaseHandler):
    ...
    def post(self, user, date):
        ...
        calculator = AttendanceCalculator(self.mdb, user, month)
        ...
        data = {
            ...
            "year_days_of_vacation": get_user_days_of_vacation_in_year(self.mdb, user, month),
        }
...
class WorkspanBaseHandler(BaseHandler):
    ...
    def check_vacations_conflicts(self, user, workspan):
        ...
        vacations = adb.get_user_vacations(self.mdb, user, today)
        if vacations and vacations[0]["from"] <= workspan["from"]:
            return False

        return True

    def check_workspans_conflicts(self, user, workspan):
        today = workspan["from"].replace(hour=0, minute=0, second=0, microsecond=0)
        tomorow = today + relativedelta(days=1)

        todays_workspans = adb.get_user_workspans(self.mdb, user, today, tomorow)
...
class ApiAddWorkspanHandler(WorkspanBaseHandler):
    ...
    def post(self, user):
        ...
        contract_mdoc = udb.get_user_active_contract(self.mdb.users, user, today)
        workspan["contract"] = contract_mdoc["_id"] if contract_mdoc else None

        if not self.check_vacations_conflicts(user, workspan):
            raise BadInputError("Na dovolené se nepracuje.")

        if not self.check_workspans_conflicts(user, workspan):
            raise BadInputError("Časový konflikt s jinou prací.")

        print("ukladam workspan", workspan)

        adb.add_user_workspan(self.mdb, user, workspan)
    ...
    def post(self, user):
        ...
        for day_dict in form_data:
            ...
            for today_workspan in todays_workspans:
                adb.delete_user_workspan(self.mdb, user, today_workspan["_id"])
...
class ApiAddVacationHandler(BaseHandler):
    ...
    def post(self, user):
        ...
        other_vacations = adb.get_user_vacations(self.mdb, user, vacation["from"])
        ...
        adb.add_user_vacation(self.mdb, user, vacation)
...
class ApiInterruptVacationHandler(BaseHandler):
    ...
    def post(self, user):
        ...
        vacation_mdoc = adb.get_user_vacation_by_id(self.mdb, user, data["_id"])
        ...
        adb.interrupt_user_vacation(self.mdb, user, data["_id"], interruption_date)
...
class ApiDeleteWorkspanHandler(BaseHandler):
    ...
    def post(self, user):
        ...
        adb.delete_user_workspan(self.mdb, user, workspan_id)
